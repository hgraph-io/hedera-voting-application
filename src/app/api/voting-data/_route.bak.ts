...
export async function GET(request: Request) {
  const getVotingData = async (clientAccountId?: string) => {
    ...
    if (
      votingQueryResponse.data &&
      votingQueryResponse.data.data &&
      votingQueryResponse.data.data.topic_message
    ) {
      ...
      if (clientAccountId) {
        messageObjArray = initialMessageObjArray.filter(
          (obj) => obj.accountId === clientAccountId
        );
      }
  ...
  export default async function handler(req, res) {
    ...
    let ballots = initBallots.map((ballot) => {
      ...
      for (let index = 0; index < ballot.votes.length; index++) {
        ...
        ballot.results.key[vote.choice] = ballot.results.key[vote.choice]
          ? ballot.results.key[vote.choice] + 1
          : 1;
      }

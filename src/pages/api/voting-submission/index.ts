...
const getVotingStatusByAccountId = async (clientAccountId: string) => {
  ...
  const votingQueryResponse = await createPostAxiosInstance(
    votingEndpoint,
    nftVotingQuery,
    votingHeaders
  );
  ...
  const messageObjArray = clientAccountId
    ? initialMessageObjArray.filter((obj) => obj.accountId === clientAccountId)
    : [];
  return { messageObjArray, initialMessageObjArray };
};

const getFilterVotes = (
  obj: any,
  accountId: string,
  ballotId: string,
  serialNumber: string,
...
export default async function handler(req: any, res: any) {
  ...
  const voting = await votingSubmission({
    tokenId,
    serialNumber,
    accountId,
    ballotId,

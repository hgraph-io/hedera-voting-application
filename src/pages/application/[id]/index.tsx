...
async function fetchVotes(topicId: number, lastSequenceNumber = 0, limit = 1000) {
  ...
  const decodedVotes = initVotes.map((v) => { return {...v, ...JSON.parse(getMetadataString(v.message)), message: null}})
...
const Proposal: React.FC<{ applicationData: any[], votes:any[] }> = ({ applicationData, votes }) => {
